...
my $data = do { local $RS = undef; <> };    # Slurp input file

my @elves = map {    # Split input into a list of array refs
    [ split / \n /x ]    ## no critic (RegularExpressions)
} split / \n \n /x, $data;    ## no critic (RegularExpressions)

my $high_calories = 0;

for my $elf_arr_ref (@elves) {
    my $total = sum @{$elf_arr_ref};    # Total each list

    if ( $total > $high_calories ) {    # Return highest total
        $high_calories = $total;
    }
}
