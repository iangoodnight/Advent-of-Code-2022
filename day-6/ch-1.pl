...
sub main {
    if ( !( -p STDIN ) ) {    # Check for input
        print "Usage:\n\$> cat input.txt | ./ch-1.pl\n" or croak $ERRNO;
        exit 0;
    }
    my $message = do { local $RS = undef; <> };    # slurp
    if (
        $message =~ m{(
            ( [[:lower:]] )     # input should be all lower, but just in case
            (?! \2 )            # not repeated
            ( [[:lower:]] )     # next char
            (?! \2 | \3 )       # no repeats
            ( [[:lower:]] )     # and so on
      ...
      )
    {
        ...
        my $chars_required = $index + length $1;    # add width of packet

        print "$chars_required characters processed\n" or croak $ERRNO;   # 1566
    }
    else { print "No start-of-packet marker found\n" or croak $ERRNO; }
    return;
}
